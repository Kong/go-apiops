...
If no selectors are given, all Kong entities will be selected.`,
	...
	Example: "  # clear tags 'tag1' and 'tag2' from all services in file 'kong.yml'\n" +
		"  cat kong.yml | deck remove-tags --selector='services[*]' tag1 tag2\n" +
		"\n" +
		"  # clear all tags except 'tag1' and 'tag2' from the file 'kong.yml'\n" +
		"  cat kong.yml | deck remove-tags --keep-only tag1 tag2",
}
...
func init() {
	...
	removeTagsCmd.Flags().StringP("format", "", string(filebasics.OutputFormatYaml), "output format: "+
